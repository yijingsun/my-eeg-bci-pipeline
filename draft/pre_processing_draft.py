...
session = "T"
file_type = "gdf"
file_path = f"data/{dataset_name}/raw/{subject_id}{session}.{file_type}"
...
raw = mne.io.read_raw_gdf(file_path,
                              eog=DEFAULT_EOG_CHANNELS,
                              preload=True,
                              verbose=False)
raw.rename_channels(STANDARD_1020_MAPPING)
...
# drop bad trials from epochs
# reject_criteria = dict(eeg=100e-6)  # 100 µV, 可以根据需要调整
# epochs.drop_bad(reject=reject_criteria, flat=None, verbose=False)
# print("bad trials dropped from epochs!")
...
print("event distribution:", np.bincount(epochs.events[:, 2][1:])) # [ 0 72 72 72 72] -> [ 72 72 72 72 ]
print("预处理完成!")

# 保存预处理后的epochs数据, 供后续分析和建模使用
epoch_file_dir = f"data/{dataset_name}/epochs"

if not os.path.exists(epoch_file_dir):
    os.makedirs(epoch_file_dir)

epoch_file_type = "fif"
epoch_file_path = f"{epoch_file_dir}/{subject_id}{session}_epo.{epoch_file_type}"
...
print(f"epochs 数据已保存到: {epoch_file_path}, subject_id: {subject_id}, session: {session}")